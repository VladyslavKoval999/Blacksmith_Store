...
namespace Blacksmith_Store
{
    public partial class FormReport : Form
    {
        ...
        {
            string dbPath = @"D:\Все для навчання\4_Курс\Blacksmith_Store\Blacksmith_Store\bin\Debug\Blacksmith_StoreBD";
            string sql = @"
            SELECT 
                p.product_id,
                p.name,
            ...
            LEFT JOIN product_subtypes s ON p.subtype_id = s.subtype_id";

            ClassDataBase db = new ClassDataBase();
            db.Execute(dbPath, sql, ref productList);

            ClassSerialiaze.SerializeToXml(ref productList, "products.xml");

            new FormReportView().ShowDialog();
        }
    }
}
